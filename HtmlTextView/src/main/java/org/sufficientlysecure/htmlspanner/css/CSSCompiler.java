package org.sufficientlysecure.htmlspanner.css;
...
public class CSSCompiler {
    ...
    public static Integer parseCSSColor(String colorString) {
        ...
        if (colorString.length() == 4 && colorString.startsWith("#")) {
            ...
            //rgb(230, 0, 0) 剔除非数字
            final Matcher matcher = Pattern.compile("[^0-9]").matcher(colorString);
            final List<String> splitList = new ArrayList<>(Arrays.asList(matcher.replaceAll(" ").split(" ")));
            List<String> remove = new ArrayList<>();
            remove.add("");
            splitList.removeAll(remove);

            if (splitList.size() >= 4) {
                return Color.argb(Integer.parseInt(splitList.get(0)), Integer.parseInt(splitList.get(1)), Integer.parseInt(splitList.get(2)), Integer.parseInt(splitList.get(3)));
            } else if (splitList.size() >= 3) {
                return Color.rgb(Integer.parseInt(splitList.get(0)), Integer.parseInt(splitList.get(1)), Integer.parseInt(splitList.get(2)));
            }
